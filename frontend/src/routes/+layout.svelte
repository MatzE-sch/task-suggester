<script lang="ts">
  ...
  import { isOnline, pendingMutations, replayMutations, clearQueue } from '$lib/stores/offline';
  ...
  function handleLogout() {
    logout();
    user.set(null);
    // Account-Daten nicht in den Gastmodus leaken; Block-Settings (Geräte-Config) bleiben
    clearDataCaches();
    clearUserCache();
    clearQueue();
...
<div class="flex-1 flex flex-col md:block overflow-hidden">
  ...
  <aside class="hidden md:flex flex-col fixed left-0 top-0 bottom-0 w-52 bg-neutral-950 border-r border-neutral-800 p-5 z-20">
    ...
    <div class="border-t border-neutral-800 pt-4 space-y-1 text-sm">
      <p class="text-xs text-neutral-500 px-1">{$user?.username ?? 'Nicht angemeldet'}</p>
      ...
      {:else}
        <button
          onclick={() => goto('/login')}
          class="w-full text-left px-3 py-2 rounded-xl text-indigo-400 hover:text-indigo-300 hover:bg-neutral-900 transition-colors text-sm"
        >Login</button>
      {/if}
    </div>
  </aside>
  ...
  <header class="md:hidden shrink-0 z-10 bg-neutral-950 border-b border-neutral-800 px-4 py-3 flex items-center justify-between relative">
    ...
    <button
      ...
      class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold text-white {$isLoggedIn ? 'bg-indigo-600' : 'bg-neutral-700'}"
      aria-label="Menü"
    >
      {#if $isLoggedIn}
        {$user?.username?.[0]?.toUpperCase() ?? '?'}
      {:else}
        <svg viewBox="0 0 24 24" class="w-4 h-4" fill="currentColor"><path d="M12 12c2.2 0 4-1.8 4-4s-1.8-4-4-4-4 1.8-4 4 1.8 4 4 4zm0 2c-2.7 0-8 1.3-8 4v2h16v-2c0-2.7-5.3-4-8-4z"/></svg>
      {/if}
    </button>
    ...
    {#if showMobileMenu}
      <div
        ...
        role="menu"
      >
        <span class="px-4 pt-2 text-xs text-neutral-500">{$user?.username ?? 'Nicht angemeldet'}</span>
        {#if PUBLIC_BUILD_TIME}
        ...
        {#if isNative()}
          <button onclick={() => { showMobileMenu = false; goto('/blocking'); }} class="px-4 py-2.5 text-sm text-neutral-300 hover:text-white hover:bg-neutral-800 transition-colors text-left">
            ⛨ Blocken
          </button>
        {/if}
        {#if $isLoggedIn}
        ...
        {:else}
          <button onclick={() => { showMobileMenu = false; goto('/login'); }} class="px-4 py-2.5 text-sm text-indigo-400 hover:text-indigo-300 hover:bg-neutral-800 transition-colors text-left">
            Login
          </button>
        {/if}
      </div>
